...
singleSim_OnlyCulex <- function(lambda_C, rho_max_C,
                           A_C, kappa_C, q_divided, livestocktotal) {
  ...
  if (modeloutput == "allMozadultsmeanlast28days") {
    ...
    assign("q_divided", value = q_divided, pos = 2, inherits = TRUE)
    assign("livestocktotal", value = livestocktotal, pos = 2, inherits = TRUE)
...
singleSim_OnlyAedes <- function(lambda_A, rho_max_A,
                                A_A, kappa_A, q_divided, livestocktotal) {
