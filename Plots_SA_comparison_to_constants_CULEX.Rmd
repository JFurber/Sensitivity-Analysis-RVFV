...
csv_list_sims <- list.files(
  # path = "Output/SobolIndices/Culex/Temperature",
  # path = "Output/SobolIndices/Culex/WaterBodies",
  path = "Output/SobolIndices/Culex/PropFind",
  pattern = ".csv$", 
  full.names = TRUE
)
...
csv_imports$temperature <- as.numeric(gsub(".*C([0-9]+)temp_.*", "\\1", csv_imports$path))
...
# write.csv(csv_imports_temp, file = paste0("Output/SobolIndices/Culex/Temperature/", "Temperature_compare.csv"))
# write.csv(csv_imports_water, file = paste0("Output/SobolIndices/Culex/WaterBodies/", "Water_compare.csv"))
write.csv(csv_imports_prop, file = paste0("Output/SobolIndices/Culex/PropFind/", "Prop_compare.csv"))
```
...
# ggsave(plot_temperature, filename = "Output/SobolIndices/Culex/Culex_temperature.png",
#        width = 6, height = 2)
# ggsave(plot_temperature,
#              filename = "Output/SobolIndices/Culex/Culex_temperature.pdf",
#              width = 6, height = 2,device=pdf)

# ggsave(plot_water, filename = "Output/SobolIndices/Culex/Culex_water.png",
#        width = 6, height = 2)
# ggsave(plot_water,
#              filename = "Output/SobolIndices/Culex/Culex_water.pdf",
#              width = 6, height = 2,device=pdf)
# 
ggsave(plot_prop, filename = "Output/SobolIndices/Culex/Culex_prop.png",
       width = 6, height = 2)
ggsave(plot_prop,
             filename = "Output/SobolIndices/Culex/Culex_prop.pdf",
             width = 6, height = 2,device=pdf)
```


```{r - if importing from csv files}

# Plot for Temperature
plot_temperature <- ggplot(data = Temperature_compare, aes(x = temperature, y = original, col = parameters)) + 
                    geom_line() + 
                    geom_point() + 
                    facet_wrap(~sensitivity, scales = "free",
                               labeller = labeller(sensitivity = c("Si" = "First Order Index", 
                    ...
                    scale_color_viridis_d(
                     ...
                     labels = c(expression(italic(A[C])), 
                                ...
                                expression(italic(b[C])), 
                                expression(italic(rho[C])))) +
                    scale_x_continuous(breaks = unique(Temperature_compare$temperature)) +
                    theme_classic()+
                    theme(axis.title = element_text(size = 10),    # Axis labels
                          axis.text = element_text(size = 8),      # Tick labels
                          strip.text = element_text(size = 8),     # Facet titles
...
plot_water <- ggplot(data = Water_compare, aes(x = water, y = original, col = parameters)) + 
                    geom_line() + 
                    geom_point() + 
                    facet_wrap(~sensitivity, scales = "free",
                               labeller = labeller(sensitivity = c("Si" = "First Order Index", 
                    ...
                    scale_color_viridis_d(
                     ...
                     labels = c(expression(italic(A[C])), 
                                ...
                                expression(italic(b[C])), 
                                expression(italic(rho[C])))) +
                    scale_x_continuous(breaks = unique(Water_compare$water)) +
                    theme_classic()+
                    theme(axis.title = element_text(size = 10),    # Axis labels
                          axis.text = element_text(size = 8),      # Tick labels
                          strip.text = element_text(size = 8),     # Facet titles
...
plot_prop <- ggplot(data = Prop_compare, aes(x = prop, y = original, col = parameters)) + 
                    geom_line() + 
                    geom_point() + 
                    facet_wrap(~sensitivity, scales = "free",
                               labeller = labeller(sensitivity = c("Si" = "First Order Index", 
                    ...
                    scale_color_viridis_d(
                     ...
                     labels = c(expression(italic(A[C])), 
                                ...
                                expression(italic(b[C])), 
                                expression(italic(rho[C])))) +
                    scale_x_continuous(breaks = unique(Prop_compare$prop)) +
                    theme_classic()+
                    theme(axis.title = element_text(size = 10),    # Axis labels
                          axis.text = element_text(size = 8),      # Tick labels
                          strip.text = element_text(size = 8),     # Facet titles
...
combined_plot <- as.ggplot(function() {
  grid.arrange(plot_temperature, plot_water, plot_prop, ncol = 1)
})

# Save to file
ggsave("Culex_Total_Compare.png", combined_plot, width = 6, height = 6, units = "in", dpi = 300)

# Open EPS device
postscript("combined_plot.eps", width = 6, height = 6, paper = "special", horizontal = FALSE, onefile = FALSE)
# Draw your combined plot
grid.arrange(plot_temperature, plot_water, plot_prop, ncol = 1)
# Close device
dev.off()
